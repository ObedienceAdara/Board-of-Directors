...
def build_provenance_ledger(
    state: dict[str, Any], generated_at: str | None = None
) -> dict[str, Any]:
    ...
    def add_evidence(
        ...
        matching_trace = next(
            (
                trace
                for trace in traces.get(agent, [])
                if str(trace.get("url", "")).strip() == url
            ),
            None,
        )
        if matching_trace:
            ...
            publisher = _text(
                item.get("publisher") or item.get("source_publisher") or item.get("source_name"),
                300,
            ) or None
    ...
    for agent in (
        ...
        # Component claims preserve the operands behind aggregate figures.
        ...
        raw_evidence = analysis.get("evidence", []) if agent == "researcher" else []
        if isinstance(raw_evidence, list):
            for index, item in enumerate(raw_evidence[:60], start=1):
                if isinstance(item, dict):
        ...
        for raw in raw_claims[:MAX_LEDGER_CLAIMS]:
            ...
            # Match only evidence belonging to the current agent. This avoids
            # accidentally attributing another department's source.
            for evidence_id, ev in evidence_by_id.items():
                if ev.get("agent") == agent and _match_evidence(
                    claim_id, raw.get("value"), str(raw.get("unit", "")), ev
                ):
    ...
    # Materialize the CTO sequential phase sum as an explicit derived claim.
    cto = state.get("cto_formal", {}) or {}
    phases = cto.get("development_phases") if isinstance(cto, dict) else None
    if isinstance(phases, list):
        valid_phases = [
            x for x in phases[:30]
            if isinstance(x, dict) and _number(x.get("weeks")) is not None
        ]
        if valid_phases:
            ...
            claims.append(
                {
                    "claim_id": claim_id,
                    "claim_instance_id": f"CLAIM-{len(claims) + 1:04d}",
                    "value": round(sum(float(x["weeks"]) for x in valid_phases), 3),
                    "unit": "weeks",
    ...
    # Decision nodes are explicit provenance edges from claims into board actions.
    claim_ids = sorted({str(x["claim_id"]) for x in claims if x.get("claim_id")})
    contradictions = [
        str(x["id"])
        for x in state.get("deterministic_contradictions", []) or []
                ...
                decisions.append(
                    {
                        ...
                        "agent": "ceo",
                        "decision_text": _text(
                            issue.get("resolution") or issue.get("rationale"),
                            1500,
                        ),
    ...
    evidence_coverage = round(sourced / total, 3) if total else 0.0
...
def validate_provenance_ledger(ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    for transformation in transformations if isinstance(transformations, list) else []:
        if not isinstance(transformation, dict):
            ...
            continue
        output = transformation.get("output_claim_id")
        if output not in claim_ids:
            errors.append(f"transformation references missing output {output}")
