...
def _dependencies(claim_id: str, analysis: dict[str, Any]) -> list[str]:
    ...
    if claim_id == "finance.startup_cost":
        return [f"finance.startup_cost.item.{index}" for index, item in enumerate(analysis.get("startup_costs", [])[:50], start=1) if isinstance(item, dict) and _number(item.get("amount")) is not None]
    if claim_id == "operations.annual_payroll":
        return [f"operations.payroll.item.{index}" for index, item in enumerate(analysis.get("headcount_plan", [])[:50], start=1) if isinstance(item, dict) and _number(item.get("count")) is not None and _number(item.get("annual_salary")) is not None]
    if claim_id == "marketing.channel_allocation_total":
        return [f"marketing.channel.{index}.amount" for index, item in enumerate(analysis.get("channel_allocations", [])[:50], start=1) if isinstance(item, dict) and _number(item.get("amount")) is not None]
    if claim_id == "technical.development_phase_sum_weeks":
        return [f"technical.development_phase.{index}.weeks" for index, item in enumerate(analysis.get("development_phases", [])[:30], start=1) if isinstance(item, dict) and _number(item.get("weeks")) is not None]
    return []
...
def _match_evidence(claim_id: str, value: Any, unit: str, evidence: dict[str, Any]) -> bool:
    ...
    if expected is not None and observed is not None and abs(expected - observed) <= max(abs(expected), 1.0) * 0.001:
        if not unit or not observed_unit or unit.lower() == observed_unit:
            return True
...
def build_provenance_ledger(state: dict[str, Any], generated_at: str | None = None) -> dict[str, Any]:
    ...
    def add_source(url: str, title: str, publisher: str | None, retrieved_at: str | None, retrieval_metadata: dict[str, Any]) -> str:
        source_id = _source_key(url, title)
        source_by_id.setdefault(source_id, {"source_id": source_id, "url": url, "title": title or url, "publisher": publisher, "retrieved_at": retrieved_at, "retrieval_metadata": retrieval_metadata})
        return source_id
    ...
    def add_evidence(agent: str, item: dict[str, Any], evidence_id: str) -> None:
        ...
        matching_trace = next((trace for trace in traces.get(agent, []) if str(trace.get("url", "")).strip() == url), None)
        if matching_trace:
            retrieved_at = _text(matching_trace.get("retrieved_at"), 80) or None
            retrieval_metadata = {
                "provider": matching_trace.get("provider") or "tavily", "query": matching_trace.get("query"),
                "rank": matching_trace.get("rank"), "score": matching_trace.get("score"), "published_at": matching_trace.get("published_at"),
                "retrieval_method": "live_web_search", "retrieval_timestamp_recorded": bool(retrieved_at), "trace_observed": True,
            }
            publisher = _text(matching_trace.get("publisher"), 300) or None
        else:
            retrieved_at = _text(item.get("retrieved_at"), 80) or None
            retrieval_metadata = {
                "provider": _text(item.get("provider"), 80) or None, "query": _text(item.get("query"), 700) or None,
                "rank": item.get("rank") if isinstance(item.get("rank"), int) else None, "retrieval_method": "model_reported_metadata",
                "retrieval_timestamp_recorded": bool(retrieved_at), "trace_observed": False,
            }
        ...
        record = {
            "evidence_id": evidence_id, "source_id": source_id, "agent": agent, "evidence_type": "source_excerpt",
            "claim_id": item.get("claim_id"), "claim_text": _text(item.get("claim"), 700), "value": item.get("value"),
            "unit": _text(item.get("unit"), 80), "excerpt": _text(item.get("evidence_excerpt") or item.get("retrieved_context") or item.get("context"), 1800),
    ...
    for agent in ("researcher", "cfo", "cto", "cmo", "coo", "head_of_sales", "pm"):
        ...
        if agent == "cfo" and isinstance(analysis.get("startup_costs"), list):
            for index, item in enumerate(analysis["startup_costs"][:50], start=1):
                if isinstance(item, dict) and _number(item.get("amount")) is not None:
                    claims.append({"claim_id": f"finance.startup_cost.item.{index}", "claim_instance_id": f"CLAIM-{len(claims)+1:04d}", "value": float(item["amount"]), "unit": currency, "claim_type": "cost_component", "method": "agent_assertion", "formula": None, "dependencies": [], "evidence_refs": [], "source_refs": [], "agent": agent, "confidence": confidence, "status": "validated" if not validation.get("errors") else "validation_error", "component_name": _text(item.get("name"), 200)})
            ...
            for index, item in enumerate(analysis["headcount_plan"][:50], start=1):
                if isinstance(item, dict):
                    ...
                    if count is not None and salary is not None:
                        claims.append({"claim_id": f"operations.payroll.item.{index}", "claim_instance_id": f"CLAIM-{len(claims)+1:04d}", "value": count*salary, "unit": currency, "claim_type": "payroll_component", "method": "derived", "formula": f"{count:g} * {salary:g}", "dependencies": [], "evidence_refs": [], "source_refs": [], "agent": agent, "confidence": confidence, "status": "validated" if not validation.get("errors") else "validation_error", "component_name": _text(item.get("role"), 200)})
        if agent == "cmo" and isinstance(analysis.get("channel_allocations"), list):
            for index, item in enumerate(analysis["channel_allocations"][:50], start=1):
                if isinstance(item, dict) and _number(item.get("amount")) is not None:
                    claims.append({"claim_id": f"marketing.channel.{index}.amount", "claim_instance_id": f"CLAIM-{len(claims)+1:04d}", "value": float(item["amount"]), "unit": currency, "claim_type": "marketing_component", "method": "agent_assertion", "formula": None, "dependencies": [], "evidence_refs": [], "source_refs": [], "agent": agent, "confidence": confidence, "status": "validated" if not validation.get("errors") else "validation_error", "component_name": _text(item.get("channel"), 200)})
        if agent == "cto" and isinstance(analysis.get("development_phases"), list):
            for index, item in enumerate(analysis["development_phases"][:30], start=1):
                if isinstance(item, dict) and _number(item.get("weeks")) is not None:
                    claims.append({"claim_id": f"technical.development_phase.{index}.weeks", "claim_instance_id": f"CLAIM-{len(claims)+1:04d}", "value": float(item["weeks"]), "unit": "weeks", "claim_type": "timeline_component", "method": "agent_assertion", "formula": None, "dependencies": [], "evidence_refs": [], "source_refs": [], "agent": agent, "confidence": confidence, "status": "validated" if not validation.get("errors") else "validation_error", "component_name": _text(item.get("name"), 200)})

        if agent == "researcher" and isinstance(analysis.get("evidence"), list):
        ...
        for raw in raw_claims[:MAX_LEDGER_CLAIMS]:
            ...
            for evidence_id, ev in evidence_by_id.items():
                if ev.get("agent") == agent and _match_evidence(claim_id, raw.get("value"), str(raw.get("unit", "")), ev):
                    ...
                    method = "reported"
                    break
            claims.append({"claim_id": claim_id, "claim_instance_id": f"CLAIM-{len(claims)+1:04d}", "value": raw.get("value"), "unit": _text(raw.get("unit"), 80), "claim_type": _claim_type(claim_id), "method": method, "formula": formula, "dependencies": dependencies, "evidence_refs": evidence_refs, "source_refs": source_refs, "agent": agent, "confidence": _confidence(raw.get("confidence"), confidence), "status": "validation_error" if validation.get("errors") else "validated"})
            if method == "derived":
                transformations.append({"transformation_id": f"TX-{len(transformations)+1:04d}", "output_claim_id": claim_id, "operation": "deterministic_formula", "formula": formula, "dependency_claims": dependencies, "agent": agent, "deterministic": True, "created_at": captured_at})
    ...
    contradiction_refs = [str(x["id"]) for x in state.get("deterministic_contradictions", []) or [] if isinstance(x, dict) and x.get("id")]
    decisions: list[dict[str, Any]] = [{"decision_id": "board.recommendation", "decision_type": "board_recommendation", "agent": "ceo", "decision_text": _text(state.get("final_board_report"), 2400), "claim_refs": claim_ids, "contradiction_refs": contradiction_refs, "consistency_status": _text(state.get("consistency_status"), 80) or "NOT_RUN", "created_at": captured_at}]
    ...
    total, sourced = len(claims), sum(1 for x in claims if x.get("source_refs"))
    derived, asserted = sum(1 for x in claims if x.get("method") == "derived"), sum(1 for x in claims if x.get("method") == "agent_assertion")
    invalid = sum(1 for x in claims if x.get("status") == "validation_error")
    return {
        "schema_version": PROVENANCE_SCHEMA_VERSION,
        "generated_at": captured_at,
        "claims": claims[:MAX_LEDGER_CLAIMS], "evidence": evidence[:MAX_LEDGER_EVIDENCE],
        "sources": list(source_by_id.values())[:MAX_LEDGER_SOURCES], "transformations": transformations[:MAX_LEDGER_TRANSFORMATIONS],
        "decisions": decisions[:MAX_LEDGER_DECISIONS],
        "summary": {"total_claims": total, "sourced_claims": sourced, "derived_claims": derived, "agent_assertions": asserted, "validation_error_claims": invalid, "evidence_records": len(evidence), "source_records": len(source_by_id), "transformation_records": len(transformations), "decision_records": len(decisions), "evidence_coverage_ratio": round(sourced / total, 3) if total else 0.0, "unsupported_claims": max(total - sourced - derived, 0)},
    }
