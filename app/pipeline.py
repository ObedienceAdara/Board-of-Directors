...
def initialize_state(brief: dict[str, Any]) -> BoardState:
    ...
    for agent in AGENT_ORDER:
        state.update({
            ...
            f"{agent}_revisions": 0, f"{agent}_passed": False, f"{agent}_feedback": "", f"{agent}_execution_error": "",
            f"{agent}_forced_accept": False, f"{agent}_evaluation": {},
        })
    state.update({
        "ceo_task_assignments": "", "research_report": "", "financial_plan": "", "tech_plan": "", "marketing_plan": "",
        "operations_plan": "", "sales_strategy": "", "product_roadmap": "", "scheduler_status": {}, "scheduler_events": [],
...
def _formal_stage_ok(state: BoardState) -> bool:
    ...
    if not isinstance(statuses, dict) or any(statuses.get(agent) != "passed" for agent in AGENT_ORDER):
        state.setdefault("pipeline_errors", []).append({
            ...
            "message": f"Formal board incomplete; failed={state.get('scheduler_failed_agents', [])}, blocked={state.get('scheduler_blocked_agents', [])}.",
        })
        return False
    return True
...
def _calculation_lineage(state: BoardState) -> list[dict[str, Any]]:
    ...
    values = {
        "finance.12_month_revenue": domains["finance"].get("12_month_revenue"), "finance.gross_margin": domains["finance"].get("gross_margin"),
        "finance.contribution_margin": domains["finance"].get("contribution_margin"), "finance.net_burn": domains["finance"].get("net_burn"),
        "finance.runway_months": domains["finance"].get("runway_months"), "finance.break_even_month": domains["finance"].get("break_even_month"),
        "sales.12_month_revenue": domains["sales"].get("12_month_revenue"), "sales.required_annual_sales": domains["sales"].get("required_annual_sales"),
        "operations.12_month_payroll": domains["operations"].get("12_month_payroll"), "technical.delivery_duration_weeks": domains["technical"].get("delivery_duration_weeks"),
        "product.priority_scores": [item.get("priority_score") for item in domains["product"].get("features", []) if isinstance(item, dict)],
    }
    formulas = {
        "finance.12_month_revenue": "sum(finance.months[*].revenue)", "finance.gross_margin": "(revenue-cogs)/revenue",
        "finance.contribution_margin": "revenue-cogs-marketing", "finance.net_burn": "operating_costs-revenue",
        "finance.runway_months": "cash-depletion simulation", "finance.break_even_month": "first forecast month with net_burn <= 0",
        "sales.12_month_revenue": "sum(sales.months[*].revenue)", "sales.required_annual_sales": "annual_revenue_target / annual_revenue_per_active_customer",
        "operations.12_month_payroll": "sum(monthly payroll after hire dates)", "technical.delivery_duration_weeks": "dependency-constrained shared-capacity schedule",
    ...
    return [{"calculation_id": calculation_id, "domain": calculation_id.split(".", 1)[0], "engine": domains[calculation_id.split(".", 1)[0]].get("model", "deterministic_unknown"), "value": value, "formula": formulas[calculation_id], "agent_inputs": inputs[calculation_id.split(".", 1)[0]]} for calculation_id, value in values.items()]
