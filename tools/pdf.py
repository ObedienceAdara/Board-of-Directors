...
def generate_pdf(board_data: dict[str, Any], output_path: str = "board_report.pdf") -> str:
    """Generate an 11-page decision-grade board report."""
    pages = list(board_data.get("pages", []))
    if not pages:
        raise ValueError("PDF report requires at least one page model")
    if len(pages) > 12:
        raise ValueError("Executive PDF is intentionally capped at 12 pages")
    ...
    title_style = ParagraphStyle(
        ...
        fontSize=19, leading=23, textColor=colors.HexColor("#172033"), spaceAfter=5,
    )
    subtitle_style = ParagraphStyle(
        "Subtitle", parent=styles["Normal"], fontName="Helvetica",
        fontSize=9.5, leading=13, textColor=colors.HexColor("#667085"), spaceAfter=10,
    )
    section_style = ParagraphStyle(
        "BlockTitle", parent=styles["Heading2"], fontName="Helvetica-Bold",
        fontSize=10, leading=13, textColor=colors.HexColor("#344054"), spaceBefore=6, spaceAfter=4,
    )
    body_style = ParagraphStyle(
        "Body", parent=styles["BodyText"], fontName="Helvetica",
        fontSize=9.5, leading=13.3, textColor=colors.HexColor("#243042"), spaceAfter=4,
    )
    bullet_style = ParagraphStyle(
        "Bullet", parent=body_style, leftIndent=10, firstLineIndent=-5, spaceAfter=2.5,
    )
    cover_title = ParagraphStyle(
        "CoverTitle", parent=title_style, fontSize=25, leading=30, alignment=TA_CENTER, spaceAfter=12,
    )
    cover_subtitle = ParagraphStyle(
        "CoverSubtitle", parent=subtitle_style, fontSize=11.5, leading=16, alignment=TA_CENTER,
    )
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Board Report — {board_data.get('idea', 'Business Idea')}",
        author="Plex Hedge Board of Directors AI",
    )

    story: list[Any] = []
    idea = str(board_data.get("idea", "Business Idea"))[:180]

    # Cover is page 1. The remaining page models become pages 2..N.
    story.extend([
        Spacer(1, 5.0 * cm),
        Paragraph("BOARD OF DIRECTORS", cover_subtitle),
        Paragraph("STRATEGIC DECISION REPORT", cover_title),
        HRFlowable(width="72%", thickness=1.5, color=colors.HexColor("#344054"), hAlign="CENTER"),
        Spacer(1, 0.8 * cm),
        Paragraph(_inline(idea), cover_subtitle),
        Spacer(1, 1.0 * cm),
        Paragraph(f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}", cover_subtitle),
        Spacer(1, 6.4 * cm),
        Paragraph("Decision-grade synthesis of the board's research, economics, engineering, market, sales, operations and product analysis.", cover_subtitle),
        PageBreak(),
    ])
    ...
    for index, page in enumerate(pages, start=2):
        ...
        story.append(HRFlowable(width="100%", thickness=0.8, color=colors.HexColor("#D9DDE5")))
