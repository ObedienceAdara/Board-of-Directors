...
MAX_CLAIMS = 80
MONTHS = 12
PRICE_PERIODS = {"month", "monthly", "year", "annual", "yearly", "one_time", "once", "transaction", "per_transaction"}
BUDGET_PERIODS = {"month", "monthly", "quarter", "quarterly", "year", "annual", "yearly"}
...
def _claim(claim_id: str, value: Any, *, unit: str = "", source: str = "derived", confidence: float = 0.7) -> dict[str, Any]:
    return {"id": claim_id, "value": value, "unit": unit, "source": source, "confidence": max(0.0, min(1.0, float(confidence)))}
...
def _validate_schedule(value: Any, key: str, errors: list[str]) -> list[Any]:
    ...
    if not isinstance(value, list):
        ...
        return []
    if not value:
        errors.append(f"{key} cannot be empty when supplied")
        return []
    if len(value) > MONTHS:
...
def validate_research(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in evidence[:60]:
        ...
        if claim_id is not None and not str(claim_id).strip():
            errors.append("claim_id must be a non-empty string or null")
        if claim_id and not str(item.get("source_url", "")).strip():
    ...
    return {"valid": not errors, "errors": errors, "warnings": warnings, "claims": claims, "evidence_count": len(evidence)}
...
def validate_cfo(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("monthly_infrastructure_schedule", "monthly_marketing_schedule", "monthly_other_opex_schedule"):
        _validate_schedule(analysis.get(key), key, errors)
...
def validate_cto(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in team[:30]:
        ...
        if count is not None:
            team_capacity += count * (weekly_hours / 40.0 if weekly_hours is not None else (weekly_weeks or 0.0))
...
def validate_cmo(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    budget_period = str(analysis.get("budget_period", "")).strip().lower()
    if budget_period not in BUDGET_PERIODS:
        errors.append("budget_period must be month, quarter, or year")
...
def validate_sales(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("qualification_rate", "opportunity_rate", "close_rate", "monthly_churn_rate"):
        value = _number(analysis.get(key))
        if value is None:
    ...
    price_period = str(analysis.get("price_period", "")).strip().lower()
    ...
    if annual_target is not None and price is not None:
        normalized_period = {"monthly": "month", "annual": "year", "yearly": "year", "once": "one_time", "per_transaction": "transaction"}.get(price_period, price_period)
        required = annual_target / (price * 12 if normalized_period == "month" else price if normalized_period in {"year", "one_time", "transaction"} else 1)
        if required_declared is not None and normalized_period in {"month", "year", "one_time", "transaction"} and not _close(required_declared, required, tolerance=0.02):
            errors.append("required_annual_sales/customers is inconsistent with annual_revenue_target and price_period")
    claims = []
    if price is not None: claims.append(_claim("pricing.primary_price", price, unit=currency))
    if annual_target is not None: claims.append(_claim("sales.annual_revenue_target", annual_target, unit=currency))
...
def validate_coo(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for person in hires[:50]:
        if not isinstance(person, dict):
            ...
            continue
        count = _number(person.get("count")); salary = _number(person.get("annual_salary")); start = _number(person.get("start_month")); ramp = _number(person.get("ramp_months")); hours = _number(person.get("monthly_capacity_hours"))
        if count is None or count < 0: errors.append("headcount entries require non-negative count")
        if salary is None or salary < 0: errors.append("headcount entries require non-negative annual_salary")
        if start is None or not 1 <= start <= MONTHS: errors.append(f"start_month must be between 1 and {MONTHS}")
        if ramp is not None and ramp < 0: errors.append("ramp_months cannot be negative")
        if hours is not None and hours < 0: errors.append("monthly_capacity_hours cannot be negative")
        if count is not None and salary is not None: annual_payroll += count * salary
    workload = _required_number(analysis, "workload_hours_per_customer", errors, minimum=EPSILON)
    productive = _number(analysis.get("productive_hours_per_employee"))
    if productive is not None and productive < 0: errors.append("productive_hours_per_employee cannot be negative")
    ...
    return {"valid": not errors, "errors": errors, "warnings": warnings, "claims": claims, "derived": {"annual_payroll": round(annual_payroll, 2), "workload_hours_per_customer": workload}}
...
def validate_pm(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    for feature in features[:50]:
        if not isinstance(feature, dict):
            ...
            continue
        impact = _number(feature.get("impact")); effort = _number(feature.get("effort")); strategic = _number(feature.get("strategic_weight")); dependency = _number(feature.get("dependency_factor"))
        if impact is None or impact < 0: errors.append("feature impact must be non-negative")
        if effort is None or effort <= 0: errors.append("feature effort must be positive")
        if strategic is not None and strategic < 0: errors.append("feature strategic_weight cannot be negative")
        if dependency is not None and dependency <= 0: errors.append("feature dependency_factor must be positive")
    claims = [_claim("product.mvp_feature_count", len(features), unit="features")]
    return {"valid": not errors, "errors": errors, "warnings": warnings, "claims": claims, "derived": {}}


VALIDATORS = {"researcher": validate_research, "cfo": validate_cfo, "cto": validate_cto, "cmo": validate_cmo, "head_of_sales": validate_sales, "coo": validate_coo, "pm": validate_pm}
...
def _flatten_claims(formal_by_agent: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for agent, analysis in formal_by_agent.items():
        for item in analysis.get("claims", []) if isinstance(analysis, dict) else []:
            if isinstance(item, dict) and item.get("id") is not None:
                row = dict(item); row["agent"] = agent; claims.append(row)
    return claims


def detect_contradictions(formal_by_agent: dict[str, dict[str, Any]]) -> dict[str, Any]:
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    claims = _flatten_claims(formal_by_agent)
    for item in claims: grouped[str(item["id"])].append(item)
    ...
    for claim_id, items in grouped.items():
        ...
        for left_index, left in enumerate(numeric):
            for right in numeric[left_index + 1:]:
                ...
                unit_left, unit_right = str(left.get("unit", "")), str(right.get("unit", ""))
                if unit_left and unit_right and unit_left != unit_right: continue
                a, b = float(left["value"]), float(right["value"])
                if not _close(a, b, tolerance=0.05):
                    contradictions.append({"id": f"C{len(contradictions)+1:03d}", "claim_id": claim_id, "type": "numeric_conflict", "severity": "high" if abs(a-b)/max(abs(a), abs(b), 1.0) >= 0.25 else "medium", "left": left, "right": right, "difference_ratio": abs(a-b)/max(abs(a), abs(b), 1.0)})
    return {"contradictions": contradictions, "contradiction_count": len(contradictions), "claims_checked": len(claims), "claim_groups": len(grouped)}
