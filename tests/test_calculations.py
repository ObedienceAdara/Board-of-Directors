...
def test_financial_engine_calculates_pnl_cash_and_break_even() -> None:
    result = calculate_financial_model({"starting_cash": 1000, "price": 100, "price_period": "month", "starting_customers": 0, "monthly_new_customers": [5] * 12, "churn_rate": 0, "cogs_per_customer": 10, "cogs_percent_revenue": 0, "payroll_monthly": [100] * 12, "infrastructure_monthly": [20] * 12, "marketing_monthly": [30] * 12, "other_monthly": [0] * 12})
...
def test_financial_model_accounts_for_startup_cost_before_month_one() -> None:
    result = calculate_financial_model({"starting_cash": 5000, "startup_costs": 1500, "price": 0, "price_period": "month", "starting_customers": 0, "monthly_new_customers": [0] * 12, "payroll_monthly": [0] * 12, "infrastructure_monthly": [0] * 12, "marketing_monthly": [0] * 12, "other_monthly": [0] * 12})
...
def test_financial_runway_is_cash_depletion_based() -> None:
    result = calculate_financial_model({"starting_cash": 250, "price": 0, "price_period": "month", "starting_customers": 0, "monthly_new_customers": [0] * 12, "payroll_monthly": [100] * 12, "infrastructure_monthly": [0] * 12, "marketing_monthly": [0] * 12, "other_monthly": [0] * 12})
...
def test_annual_recurring_price_is_normalized_to_monthly_revenue() -> None:
    result = calculate_sales_funnel({"monthly_traffic": [10] * 12, "qualification_rate": 1, "opportunity_rate": 1, "close_rate": 1, "price": 1200, "price_period": "year", "starting_customers": 0, "monthly_churn_rate": 0, "annual_revenue_target": 12000})
...
def test_sales_funnel_connects_traffic_to_customers_and_revenue() -> None:
    result = calculate_sales_funnel({"monthly_traffic": [100] * 12, "qualification_rate": 0.5, "opportunity_rate": 0.5, "close_rate": 0.2, "price": 100, "price_period": "month", "starting_customers": 0, "monthly_churn_rate": 0, "annual_revenue_target": 12000})
...
def test_workforce_model_applies_start_dates_and_ramp() -> None:
    result = calculate_workforce_capacity({"headcount_plan": [{"role": "Engineer", "count": 2, "annual_salary": 1200, "start_month": 2, "ramp_months": 2, "monthly_capacity_hours": 100}], "workload_hours_per_customer": 10, "required_monthly_customers": 15})
...
def test_delivery_model_parallelizes_independent_phases() -> None:
    result = calculate_delivery_model({"engineering_team": [{"role": "Engineer", "count": 2, "weekly_capacity_weeks": 1}], "development_phases": [{"name": "A", "weeks": 4, "dependencies": []}, {"name": "B", "weeks": 4, "dependencies": []}, {"name": "C", "weeks": 4, "dependencies": ["A", "B"]}], "schedule_buffer": 0})
